...
DRIVER_PV(struct pc_pv_s);

static void pc_gpio_read(struct device_s *dev)
{
  struct pc_pv_s *pv = dev->drv_pv;
...
static void pc_gpio_wait(struct device_s *dev)
{
  struct pc_pv_s *pv = dev->drv_pv;
...
static
void pc_update(struct device_s *dev)
{
  struct pc_pv_s *pv = dev->drv_pv;

  uint8_t last_state = pv->last_state;
  uint8_t state = pv->cur[0] & 1;

  if (!last_state && state)
    pv->count++;

  logk_trace("%d->%d, count %d", last_state, state, pv->count);

  uint32_t count = pv->count;
  pv->last_state = state;
...
static KROUTINE_EXEC(pc_gpio_done)
{
  struct pc_pv_s *pv = KROUTINE_CONTAINER(kr, *pv, gpio_rq.base.kr);
...
static DEV_VALIO_REQUEST(pc_request)
{
  ...
  uint32_t *rq_ptr = (uint32_t*)rq->data;
  uint32_t count = pv->count;

  switch (rq->type) {
  case DEVICE_VALIO_WAIT_EVENT:
    if (*rq_ptr == count) {
      logk_debug("wait %d = %d queued", *rq_ptr, count);
      rq->error = 0;
...
static DEV_VALIO_CANCEL(pc_cancel)
{
  struct device_s *dev = accessor->dev;
  struct pc_pv_s *pv = dev->drv_pv;
...
#define pc_use dev_use_generic

static DEV_INIT(pc_init)
{
  struct pc_pv_s *pv;
  error_t err;
  gpio_id_t id;
  ...
  err = device_gpio_get_setup(&pv->gpio, dev, "pin", &id, &width);
  if (err) {
    logk_error("No 'pin' param");
    goto put_gpio;
  }

  if (width != 1) {
    logk_error("Width is supposed to be 1");
    goto put_gpio;
  }

  dev_rq_queue_init(&pv->queue);
  dev_gpio_rq_init(&pv->gpio_rq, pc_gpio_done);

  pv->gpio_rq.pvdata = dev;
  pv->gpio_rq.io_first = id;
  pv->gpio_rq.io_last = id;
  ...
  logk("Pin %d mode %d", id, pv->gpio_rq.mode.mode);
...
DRIVER_DECLARE(pc_gpio_drv, 0, "Pulse counter", pc,
               DRIVER_VALIO_METHODS(pc));

DRIVER_REGISTER(pc_gpio_drv);
