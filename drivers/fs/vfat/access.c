		       ...
		       vfat_cluster_t *next_cluster_index)
{
  ...
  next_cluster = endian_le32(sector[cluster_index % (sector_size /4)]) & 0x0FFFFFFF;
  bc_release_buffer(&bc,&freelist,&request,0);
...
#endif
      if((endian_le32(buffer[index]) & 0x0FFFFFFF) == 0x00000000)
      {
	found = 1;
	buffer[index] = endian_le32(0x0FFFFFF8);
#ifdef CONFIG_DRIVER_FS_VFAT_INSTRUMENT
    wr_count ++;
#endif
	SET_BUFFER(buffers[0]->state,BC_DELAYED_WRITE);
...
#endif
  buffer = buffers[0]->content;
  val = endian_le32(buffer[current_cluster % (sector_size/4)]) & 0x0FFFFFFF;

  if( val >= 0x0FFFFFF8)
    buffer[current_cluster % (sector_size/4)] = endian_le32(*next_cluster);
  else
  {
#ifdef CONFIG_DRIVER_FS_VFAT_DEBUG
...
error_t vfat_free_fat_entry(struct vfat_context_s *ctx, vfat_cluster_t start_cluster)
{
  ...
  while((current_index > 0) && (current_index < 0x0FFFFFF7))
  {
    ...
    sector = buffers[0]->content;
    next_index = endian_le32(sector[current_index % (sector_size /4)]) & 0x0FFFFFFF;
