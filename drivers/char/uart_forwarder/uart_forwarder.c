...
static KROUTINE_EXEC(forwarder_cfg_applied)
{
  ...
  kroutine_init_deferred(&pv->uart_rq.base.kr, forwarder_cfg_changed);
...
static KROUTINE_EXEC(forwarder_cfg_changed)
{
  ...
  kroutine_init_deferred(&pv->uart_rq.base.kr, forwarder_cfg_applied);
...
static DEV_INIT(uart_forwarder_init)
{
  ...
  for (enum ep_id_e src = 0; src < EP_COUNT; ++src) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(p->in); ++i) {
      ...
      kroutine_init_deferred_seq(&ep->rq.base.kr, endpoint_read_done, &pv->seq);
...

    {
      ...
      kroutine_init_deferred_seq(&ep->rq.base.kr, endpoint_write_done, &pv->seq);
