...
int exec(lua_State *st)
{
    pthread_t b;
    char *filename = lua_tostring(st, 1);
    char *func = lua_tostring(st,2);

    uintptr_t entrypoint;
    void* handle;
    void *sym;

    if (rtld_user_dlopen(filename, &entrypoint, &handle) != 0)
    {
        printk("dlopen failed on %s...\n", filename);
        return 0;
    }
    else if (rtld_user_dlsym(handle, func, &sym) != 0)
    {
    ...
    pthread_create(&b, NULL, entrypoint, sym);
