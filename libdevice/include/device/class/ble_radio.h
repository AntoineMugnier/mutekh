...
struct dev_ble_radio_rq_s
{
  ...
  dev_timer_delay_t max_duration; /* 0 = Dont care */
  ...
  union {
    ...
    struct {
      ...
      bool_t rssi_sample;
      int16_t rssi; // in .125dB steps
      int16_t tx_power; // in .125dB steps
...
#define DRIVER_BLE_RADIO_METHODS(prefix)                                \
  ((const struct driver_class_s*)&(const struct driver_ble_radio_s){    \
