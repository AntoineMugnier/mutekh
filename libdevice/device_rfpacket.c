...
{
  RFPACKET_WAIT_IDLE,
  RFPACKET_WAIT_RX_STARTED,
  RFPACKET_WAIT_RX_PENDING,  /* dev_rfpacket_wait_rx called */
  RFPACKET_WAIT_RXING,       /* dev_rfpacket_wait_alloc called */
  RFPACKET_WAIT_RXING_END,   /* dev_rfpacket_rx_end called */
  RFPACKET_WAIT_RX_CANCEL,   /* dev_rfpacket_wait_stop called */
};
...
static KROUTINE_EXEC(dev_rfpacket_rx_end)
{
  ...
  switch (rx_rq->base.pvuint)
    {
    case RFPACKET_WAIT_RX_PENDING:
      ctx->rx.error = -ETIMEDOUT;
      /* resume the dev_rfpacket_wait_rx function */
    case RFPACKET_WAIT_RX_CANCEL:
      /* resume the dev_rfpacket_stop_rx function */
      sched_context_start(ctx->sched_ctx);
    case RFPACKET_WAIT_RX_STARTED:
      rx_rq->base.pvuint = RFPACKET_WAIT_IDLE;
      break;
    case RFPACKET_WAIT_RXING:
      rx_rq->base.pvuint = RFPACKET_WAIT_RXING_END;
      break;
    default:
      UNREACHABLE();
    }
...
static KROUTINE_EXEC(dev_rfpacket_rx_packet)
{
  ...
  switch (rx_rq->base.pvuint)
    {
    case RFPACKET_WAIT_RXING:
      rx_rq->base.pvuint = RFPACKET_WAIT_RX_STARTED;
      break;
    case RFPACKET_WAIT_RXING_END:
      rx_rq->base.pvuint = RFPACKET_WAIT_IDLE;
      break;
    default:
      UNREACHABLE();
...
dev_rfpacket_wait_alloc(struct dev_rfpacket_rq_s *rq, size_t size)
{
  ...
  switch (rx_rq->base.pvuint)
    ...
    case RFPACKET_WAIT_RX_PENDING:
      if (rx->size >= size)
        {
          rx_rq->base.pvuint = RFPACKET_WAIT_RXING;
          rx = &ctx->rx;
          rx->size = size;
          break;
        }
...
{
  struct dev_rfpacket_rq_s *rx_rq = &ctx->rx_rq;
  assert(rx_rq->base.pvuint == RFPACKET_WAIT_IDLE);
  ...
  switch (rx_rq->base.pvuint)
    ...
    case RFPACKET_WAIT_RX_STARTED:
      if (!DEVICE_OP(ctx->rf_dev, cancel, &ctx->rx_rq))
        ...
        {
          rx_rq->base.pvuint = RFPACKET_WAIT_RX_CANCEL;
                             ...
                             uint8_t *buffer, size_t *size)
{
  ...
  switch (rx_rq->base.pvuint)
    ...
    case RFPACKET_WAIT_RX_STARTED:
      rx_rq->base.pvuint = RFPACKET_WAIT_RX_PENDING;
      rx->buf = buffer;
      rx->size = *size;
