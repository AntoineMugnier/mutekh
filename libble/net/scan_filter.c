                                ...
                                struct buffer_s *buffer)
{
  ...
  case BLE_ADV_NONCONN_IND:
    ...
    if (ad_len != item->device.adv_ad_len && item->device.scan_ad_len)
      memmove(item->device.ad + ad_len, item->device.ad + item->device.adv_ad_len, item->device.scan_ad_len);
    memcpy(item->device.ad, ad, ad_len);
    item->device.adv_ad_len = ad_len;
    item->device.ad_len = ad_len + item->device.scan_ad_len;
    break;
  ...
  case BLE_SCAN_RSP:
    ...
    memcpy(item->device.ad + item->device.adv_ad_len, ad, ad_len);
    item->device.scan_ad_len = ad_len;
    item->device.ad_len = ad_len + item->device.adv_ad_len;
    break;
