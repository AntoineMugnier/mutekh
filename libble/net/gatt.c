...
static void gatt_query_handle(struct ble_gatt_s *gatt, struct ble_att_transaction_s *txn)
{
  ...
  case BLE_ATT_FIND_INFORMATION_RQT:
    err = gatt_find_information(gatt, ble_att_find_information_task_s_from_base(txn));
    break;

  case BLE_ATT_FIND_BY_TYPE_VALUE_RQT:
    err = gatt_find_by_type_value(gatt, ble_att_find_by_type_value_task_s_from_base(txn));
    break;

  case BLE_ATT_READ_BY_TYPE_RQT:
    err = gatt_read_by_type(gatt, ble_att_read_by_type_task_s_from_base(txn));
    break;

  case BLE_ATT_READ_RQT:
  case BLE_ATT_READ_BLOB_RQT:
    err = gatt_read(gatt, ble_att_read_task_s_from_base(txn));
    break;

  case BLE_ATT_READ_MULTIPLE_RQT:
    err = gatt_read_multiple(gatt, ble_att_read_multiple_task_s_from_base(txn));
    break;

  case BLE_ATT_READ_BY_GROUP_TYPE_RQT:
    err = gatt_read_by_group_type(gatt, ble_att_read_by_group_type_task_s_from_base(txn));
    break;

  case BLE_ATT_WRITE_RQT:
  case BLE_ATT_WRITE_CMD:
    err = gatt_write(gatt, ble_att_write_task_s_from_base(txn));
    break;
                                   ...
                                   const void *data, size_t size)
{
  ...
  if (mode & BLE_GATT_CCCD_INDICATION) {
    ...
    txn->authenticated = 0;
    txn->base.command = BLE_ATT_HANDLE_VALUE_INDIC;
...
error_t gatt_write(struct ble_gatt_s *gatt,
                    struct ble_att_write_task_s *task)
{
  task->base.error = _gatt_write(gatt, task->handle, task->value, task->value_size);
