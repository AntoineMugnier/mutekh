...
static KROUTINE_EXEC(link_crypto_done)
{
  ...
  if (err) {
    ...
    if (link_is_master(link)
      ...
      logk_trace("Master llcp opcode %d", opcode);
...
static void link_task_crypt(struct ble_link_s *link, struct net_task_s *task)
{
  ...
  logk_trace("%s %p -> %p %d", __func__, link->crypto_rq.in, link->crypto_rq.out, link->crypto_rq.len);
...
static void link_crypto_next(struct ble_link_s *link)
{
 ...
 again:
  ...
  if (!link->layer.parent) {
    logk_trace("%s no parent, flushing queue", __func__);
  ...
  case LINK_ENC_STOPPING:
    logk_trace("%s in state %d, getting non-data or inbound packets", __func__, link->state);
  ...
  case LINK_FAILED:
    logk_trace("%s in state %d, getting non-data outbound packets", __func__, link->state);
    GCT_FOREACH(net_task_queue, &link->queue, t,
  ...
  logk_trace("%s state %d handling %P", __func__,
          link->state,
          task->packet.buffer->data + task->packet.buffer->begin,
          task->packet.buffer->end - task->packet.buffer->begin);
                          ...
                          struct net_task_s *task)
{
  ...
  /* logk_trace("%s %d", __func__, task->type); */
  ...
  case NET_TASK_OUTBOUND:
    ...
    if (task->packet.dst_addr.unreliable && link->outbound_accepted_count < 0) {
      logk_error("dropping unreliable packet because of overflow");
      break;
    }
                        ...
                        const uint8_t iv[static 8])
{
  ...
  kroutine_init_deferred_seq(&link->crypto_rq.base.kr, link_crypto_done, &link->seq);
...
void ble_link_destroyed(struct net_layer_s *layer)
{
  ...
  logk_trace("layer %p destroyed", link);

  net_task_queue_destroy(&link->queue);
#if defined(CONFIG_BLE_CRYPTO)
  kroutine_seq_cleanup(&link->seq);
...
#if defined(CONFIG_BLE_CRYPTO)
  kroutine_seq_init(&link->seq);

  error_t err = net_layer_init_seq(&link->layer, &link_handler, scheduler,
                                   delegate, delegate_vtable, &link->seq);
#else
