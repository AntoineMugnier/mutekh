...
{
  if (params->interval < 5 || params->interval > 3200)
    return -EINVAL;

  if (params->timeout < 10 || params->timeout > 3200)
    return -EINVAL;

  if ((params->latency + 1) * params->interval * 2 > params->timeout * 8)
    return -EINVAL;
