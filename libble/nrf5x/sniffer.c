                       ...
                       struct device_icu_s *icu)
{
    ...
    timer_sink = DEVICE_OP(icu, get_endpoint, DEV_IRQ_EP_SINK, NRF5X_TIMER0);
    ...
    nrf_task_trigger(radio->timer, NRF_TIMER_STOP);
    nrf_reg_set(radio->timer, NRF_TIMER_PRESCALER, 4);
    nrf_reg_set(radio->timer, NRF_TIMER_MODE, NRF_TIMER_MODE_TIMER);
    nrf_reg_set(radio->timer, NRF_TIMER_BITMODE, NRF_TIMER_BITMODE_32);
    nrf_reg_set(radio->timer, NRF_TIMER_CC(TIMER_OVERFLOW), 0xffffffff);
    nrf_task_trigger(radio->timer, NRF_TIMER_CLEAR);
    nrf_task_trigger(radio->timer, NRF_TIMER_START);
    radio->timer_base = 0;

    nrf_it_enable(radio->timer, NRF_TIMER_COMPARE(TIMER_OVERFLOW));
    nrf_it_enable(radio->timer, NRF_TIMER_COMPARE(TIMER_DEADLINE));
...
{
    nrf_task_trigger(radio->timer2, NRF_TIMER_STOP);
    nrf_reg_set(radio->timer2, NRF_TIMER_PRESCALER, 4);
    nrf_reg_set(radio->timer2, NRF_TIMER_MODE, NRF_TIMER_MODE_TIMER);
    nrf_reg_set(radio->timer2, NRF_TIMER_BITMODE, NRF_TIMER_BITMODE_16);
    nrf_task_trigger(radio->timer2, NRF_TIMER_START);
}
                       ...
                       struct device_icu_s *icu)
{
    ...
    radio_sink = DEVICE_OP(icu, get_endpoint, DEV_IRQ_EP_SINK, NRF5X_RADIO);
    ...
    nrf_reg_set(radio->radio, NRF_RADIO_POWER, 0);

    for (uint16_t i = 0; i < 320; ++i)
        asm volatile("");

    nrf_reg_set(radio->radio, NRF_RADIO_POWER, 1);

    for (uint16_t i = 0; i < 320; ++i)
        asm volatile("");

    for (uint8_t i = 0; i < 5; ++i)
        nrf_reg_set(radio->radio, NRF_RADIO_OVERRIDE(i),
                    cpu_mem_read_32(NRF_FICR_BLE_1MBIT(i)));

    nrf_reg_set(radio->radio, NRF_RADIO_MODE, NRF_RADIO_MODE_BLE_1MBIT);
    nrf_reg_set(radio->radio, NRF_RADIO_BCC, 16);
...
{
    nrf_task_trigger(radio->timer, NRF_TIMER_CAPTURE(TIMER_VALUE));

    return ((int64_t)radio->timer_base << 32)
        + nrf_reg_get(radio->timer, NRF_TIMER_CC(TIMER_VALUE));
}
...
error_t ble_sniffer_create(size_t priv_size, struct ble_sniffer **rradio)
{
    ...
    radio->radio = nrf_peripheral_addr(NRF5X_RADIO);
    radio->timer = nrf_peripheral_addr(NRF5X_TIMER0);
    radio->timer2 = nrf_peripheral_addr(NRF5X_TIMER1);
...
static void radio_deadline_set(struct ble_sniffer *radio)
{
  ...
  if (deadline == -1) {
    nrf_it_disable(radio->timer, NRF_TIMER_COMPARE(TIMER_DEADLINE));
    return;
  }
  ...
  nrf_it_enable(radio->timer, NRF_TIMER_COMPARE(TIMER_DEADLINE));
  nrf_event_clear(radio->timer, NRF_TIMER_COMPARE(TIMER_DEADLINE));
  nrf_reg_set(radio->timer, NRF_TIMER_CC(TIMER_DEADLINE), deadline);
}
  ...
  struct rx_config *config)
{
  ...
  nrf_reg_set(radio->radio, NRF_RADIO_FREQUENCY, rq->frequency - 2400);
  nrf_reg_set(radio->radio, NRF_RADIO_DATAWHITEIV, rq->white_iv | 0x40);
  nrf_reg_set(radio->radio, NRF_RADIO_PCNF1, 0
              | ((packet->end - packet->begin - 2)
                 << NRF_RADIO_PCNF1_MAXLEN_OFFSET)
  ...
  struct rx_config *config)
{
  ...
  nrf_reg_set(radio->radio, NRF_RADIO_BASE0, rq->access << 8);
  nrf_reg_set(radio->radio, NRF_RADIO_PREFIX0, rq->access >> 24);
  nrf_reg_set(radio->radio, NRF_RADIO_CRCINIT, rq->crc_init);
  nrf_reg_set(radio->radio, NRF_RADIO_PACKETPTR,
...
void radio_configured_kick(struct ble_sniffer *radio)
{
  ...
  if (!radio->configured.rq) {
    ...
    nrf_task_trigger(radio->radio, NRF_RADIO_STOP);
    nrf_task_trigger(radio->radio, NRF_RADIO_DISABLE);
    nrf_event_clear(radio->radio, NRF_RADIO_BCMATCH);
    nrf_event_clear(radio->radio, NRF_RADIO_ADDRESS);
    nrf_event_clear(radio->radio, NRF_RADIO_PAYLOAD);
    nrf_event_clear(radio->radio, NRF_RADIO_END);
  ...
  nrf_short_set(radio->radio, 0
                | (1 << NRF_RADIO_ADDRESS_BCSTART)
                | (1 << NRF_RADIO_ADDRESS_RSSISTART));

  uint8_t st = nrf_reg_get(radio->radio, NRF_RADIO_STATE);

  switch (st) {
  case NRF_RADIO_STATE_RXRU:
    do {
      st = nrf_reg_get(radio->radio, NRF_RADIO_STATE);
    } while (st != NRF_RADIO_STATE_RXIDLE);
  ...
  case NRF_RADIO_STATE_RXIDLE:
    if (radio->configured.rq->frequency == radio->last_frequency
      ...
      //printk("st");
      nrf_task_trigger(radio->radio, NRF_RADIO_START);
      acc = 0;
      break;
    }
  ...
  default:
    //printk("re");
    nrf_task_trigger(radio->radio, NRF_RADIO_DISABLE);
    while (nrf_reg_get(radio->radio, NRF_RADIO_STATE) != NRF_RADIO_STATE_DISABLED)
      ;
  ...
  case NRF_RADIO_STATE_DISABLED:
    ...
    nrf_short_set(radio->radio, 0
                  ...
                  | (1 << NRF_RADIO_ADDRESS_BCSTART)
                  | (1 << NRF_RADIO_ADDRESS_RSSISTART));
    nrf_task_trigger(radio->radio, NRF_RADIO_RXEN);
    break;
  }

  nrf_it_enable(radio->radio, NRF_RADIO_ADDRESS);
  ...
  //printk("\n");
  while (nrf_reg_get(radio->radio, NRF_RADIO_STATE) != NRF_RADIO_STATE_RX)
    ;
...
void radio_configured_pipeline(struct ble_sniffer *radio)
{
  ...
  if (nrf_event_check(radio->radio, NRF_RADIO_END)) {
    /*
      End has trigged, but we dont know whether it was before or after
      shorts were enabled.
     */

    switch (nrf_reg_get(radio->radio, NRF_RADIO_STATE)) {
    ...
    case NRF_RADIO_STATE_RXIDLE:
      ...
      nrf_task_trigger(radio->radio, fast ? NRF_RADIO_START : NRF_RADIO_DISABLE);
      break;
    }
  }
}
...
static void radio_setup(struct ble_sniffer *radio)
{
    nrf_reg_set(radio->radio, NRF_RADIO_PCNF0, 0
                | (8 << NRF_RADIO_PCNF0_LFLEN_OFFSET)
                | (1 << NRF_RADIO_PCNF0_S0LEN_OFFSET));

    nrf_reg_set(radio->radio, NRF_RADIO_RXADDRESSES, 1 << 0);
    nrf_reg_set(radio->radio, NRF_RADIO_CRCPOLY, 0x00065B);
    nrf_reg_set(radio->radio, NRF_RADIO_CRCCNF, 0
                | NRF_RADIO_CRCCNF_SKIPADDR
                | (3 << NRF_RADIO_CRCCNF_LEN_OFFSET));
    nrf_reg_set(radio->radio, NRF_RADIO_TIFS, 150);

    nrf_ppi_setup(NRF5X_PPI, PPI_PACKET_TIMESTAMP,
                    radio->radio, NRF_RADIO_ADDRESS,
                    radio->timer, NRF_TIMER_CAPTURE(TIMER_TIMESTAMP));
    nrf_ppi_enable(NRF5X_PPI, PPI_PACKET_TIMESTAMP);

    nrf_ppi_setup(NRF5X_PPI, PPI_PACKET_ADDRESS,
                    radio->radio, NRF_RADIO_ADDRESS,
                    radio->timer2, NRF_TIMER_CAPTURE(TIMER_ADDRESS));
    nrf_ppi_enable(NRF5X_PPI, PPI_PACKET_ADDRESS);

    nrf_ppi_setup(NRF5X_PPI, PPI_PACKET_END,
                    radio->radio, NRF_RADIO_END,
                    radio->timer2, NRF_TIMER_CAPTURE(TIMER_END));
    nrf_ppi_enable(NRF5X_PPI, PPI_PACKET_END);
}
...
{
  int16_t target = nrf_reg_get(radio->timer2, NRF_TIMER_CC(TIMER_ADDRESS))
    + byte * 8 + 2;
  volatile register int16_t diff;

  do {
    nrf_task_trigger(radio->timer2, NRF_TIMER_CAPTURE(TIMER_TMP));
    diff = target - nrf_reg_get(radio->timer2, NRF_TIMER_CC(TIMER_TMP));
  } while (diff > 0 && !nrf_event_check(radio->radio, NRF_RADIO_END));
}
...
static void radio_packet_address(struct ble_sniffer *radio)
{
    ...
    ts = nrf_reg_get(radio->timer, NRF_TIMER_CC(TIMER_TIMESTAMP));
    if ((int32_t)ts >= 0
        && nrf_event_check(radio->timer, NRF_TIMER_COMPARE(TIMER_OVERFLOW)))
      ts += (int64_t)1 << 32;
    ...
    packet->rssi = nrf_reg_get(radio->radio, NRF_RADIO_RSSISAMPLE);

    /* Wait for packet size byte */
    while (!nrf_event_check(radio->radio, NRF_RADIO_BCMATCH))
      ;
    nrf_event_clear(radio->radio, NRF_RADIO_BCMATCH);
    ...
    if (now + lat * 2 < packet->timestamp + packet->duration) {
      nrf_it_enable(radio->radio, NRF_RADIO_PAYLOAD);
    } else {
      /* This is in case pipelining enable came too late, if it
         happens, END has triggered already */
      if (nrf_reg_get(radio->radio, NRF_RADIO_STATE) == NRF_RADIO_STATE_RXIDLE)
        nrf_task_trigger(radio->radio, NRF_RADIO_START);

      while (!nrf_event_check(radio->radio, NRF_RADIO_PAYLOAD))
        ;
...
static void radio_packet_done(struct ble_sniffer *radio)
{
  ...
  nrf_event_clear(radio->radio, NRF_RADIO_PAYLOAD);
  nrf_it_disable(radio->radio, NRF_RADIO_PAYLOAD);

  if (!packet)
    return;

  while (!nrf_event_check(radio->radio, NRF_RADIO_END))
    ;

  /* Now, we have actual packet data and flags */
  packet->crc = nrf_reg_get(radio->radio, NRF_RADIO_RXCRC);
  packet->crc_valid = !!nrf_reg_get(radio->radio, NRF_RADIO_CRCSTATUS);
  uint32_t delta
    = (uint16_t)nrf_reg_get(radio->timer2, NRF_TIMER_CC(TIMER_END))
    - (uint16_t)nrf_reg_get(radio->timer2, NRF_TIMER_CC(TIMER_ADDRESS));
  delta &= 0xffff;
...
static DEV_IRQ_EP_PROCESS(radio_irq)
{
  ...
  if (nrf_event_check(radio->radio, NRF_RADIO_PAYLOAD))
    radio_packet_done(radio);

  if (nrf_event_check(radio->radio, NRF_RADIO_ADDRESS))
...
static DEV_IRQ_EP_PROCESS(timer_irq)
{
  ...
  for (;;) {
    if (nrf_event_check(radio->timer, NRF_TIMER_COMPARE(TIMER_OVERFLOW))) {
      nrf_event_clear(radio->timer, NRF_TIMER_COMPARE(TIMER_OVERFLOW));
      radio->timer_base += 1;
    }

    if (nrf_event_check(radio->timer, NRF_TIMER_COMPARE(TIMER_DEADLINE))) {
      nrf_event_clear(radio->timer, NRF_TIMER_COMPARE(TIMER_DEADLINE));
      nrf_task_trigger(radio->timer, NRF_TIMER_CAPTURE(TIMER_VALUE));
      int32_t value = nrf_reg_get(radio->timer, NRF_TIMER_CC(TIMER_VALUE));
      int32_t dead = nrf_reg_get(radio->timer, NRF_TIMER_CC(TIMER_DEADLINE));
