                            ...
                            struct ble_radio_params *params)
{
  ...
  if (radio->current->data.latency_permitted
      && radio->state.data.last_rx_ok
      && !radio->current->data.md
      && buffer_queue_isempty(&radio->current->data.tx_queue)
      && buffer_queue_isempty(&radio->current->data.rx_queue))
    return 0;
...
struct buffer_s *data_payload_get(const struct ble_radio *radio)
{
  ...
  if (buffer_queue_isempty(&radio->current->data.tx_queue)) {
    ...
    // chained in tx queue.
    packet = nrf51_ble_radio_packet_alloc(radio);
                           ...
                           struct buffer_s *packet)
{
  ...
  else if (size == 2 && ble_data_llid_get(packet) == BLE_LL_DATA_CONT)
