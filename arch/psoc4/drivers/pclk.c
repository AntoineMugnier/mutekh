...
DRIVER_PV(struct psoc4_pclk_private_s
{
  struct dev_clock_sink_ep_s hfclk;
  struct dev_clock_src_ep_s src[PSOC4_PCLK_SRC_COUNT];
...
static DEV_CMU_NODE_INFO(psoc4_pclk_node_info)
{
  struct device_s *dev = accessor->dev;
  struct psoc4_pclk_private_s *pv = dev->drv_pv;
  uint32_t returned = 0
    | DEV_CMU_INFO_FREQ
    | DEV_CMU_INFO_NAME
  ...
  static const char *const node_name[] = {
    [PSOC4_PCLK_SINK_HFCLK] = "HFCLK",
  };

  if (node_id > PSOC4_PCLK_NODE_COUNT)
    return -EINVAL;

  info->name = node_name[node_id];
  info->freq = pv->hfclk_freq;

  switch ((enum psoc4_pclk_node_e)node_id) {
  case PSOC4_PCLK_DIV_INT_0A ... PSOC4_PCLK_DIV_INT_2C: {
    uint32_t id = node_id - PSOC4_PCLK_DIV_INT_0A;
    uint32_t div = pv->pclk_src[id];
    uint32_t div_ctl = cpu_mem_read_32(PCLK + PCLK_DIV_CTL_ADDR(div));

    returned |= DEV_CMU_INFO_SRC;
    returned |= DEV_CMU_INFO_PARENT;
    info->src = &pv->src[node_id];
    info->parent_id = PSOC4_PCLK_SINK_HFCLK;
    info->running = (div_ctl & PCLK_DIV_CTL_EN) && (pv->hfclk.src->flags & DEV_CLOCK_EP_CLOCK);

    if (*mask & DEV_CMU_INFO_SCALE) {
      returned |= DEV_CMU_INFO_SCALE;
      info->scale.num = 32;
      info->scale.denom = PCLK_DIV_CTL_DIV_GET(div_ctl) + 0x20;
  ...
  case PSOC4_PCLK_DIV_FRAC_0A ... PSOC4_PCLK_DIV_FRAC_0C: {
    uint32_t id = node_id - PSOC4_PCLK_DIV_FRAC_0A;
    uint32_t div = pv->pclk_src[id];
    uint32_t div_ctl = cpu_mem_read_32(PCLK + PCLK_DIV_CTL_ADDR(div));
                                    ...
                                    uint_fast8_t pclk_id)
{
  ...
  uint32_t div_ctl = cpu_mem_read_32(PCLK + PCLK_DIV_CTL_ADDR(pv->pclk_src[pclk_id]));

  return !!(div_ctl & PCLK_DIV_CTL_EN);
}
                               ...
                               uint_fast8_t pclk_id)
{
  ...
  cpu_mem_write_32(PCLK + PCLK_DIV_CMD_ADDR, 0
                   | PCLK_DIV_CMD_SEL(pv->pclk_src[pclk_id])
                   | PCLK_DIV_CMD_PA_SEL(PSOC4_PCLK_DIV_NONE)
                   | PCLK_DIV_CMD_DISABLE);

  while (cpu_mem_read_32(PCLK + PCLK_DIV_CMD_ADDR) & PCLK_DIV_CMD_DISABLE)
    ;
}
                               ...
                               uint_fast8_t pclk_id)
{
  ...
  while (cpu_mem_read_32(PCLK + PCLK_DIV_CMD_ADDR) & PCLK_DIV_CMD_ENABLE)
    ;
}
                               ...
                               uint32_t div)
{
  ...
  assert(pv->pclk_src[pclk_id] != PSOC4_PCLK_DIV_NONE);
  assert(div >= 0x20);
  ...
  if (running && (PCLK_DIV_CTL_DIV_GET(div_ctl) != div_target))
    psoc4_pclk_disable(dev, pclk_id);

  dprintk("%s %d div: %d\n", __FUNCTION__, pclk_id, div);

  cpu_mem_write_32(PCLK + PCLK_DIV_CTL_ADDR(pv->pclk_src[pclk_id]),
                   PCLK_DIV_CTL_DIV(div_target));

  if (running)
    psoc4_pclk_enable(dev, pclk_id);
}

static DEV_CLOCK_SRC_SETUP(psoc4_pclk_ep_setup)
{
  struct device_s *dev = src->dev;
  struct psoc4_pclk_private_s *pv = dev->drv_pv;
  error_t err;
  uint_fast8_t src_id = src - pv->src;
  if (src_id >= PSOC4_PCLK_SRC_COUNT)
    return -EINVAL;
...
# endif
    ...
    case DEV_CLOCK_SRC_SETUP_SCALER: {
      ...
      dprintk("%s PCLK %d div %d.%d ratio %d\n", __FUNCTION__, src_id,
             PSOC4_PCLK_DIV_TYPE(div_id), PSOC4_PCLK_DIV_NO(div_id), ratio);

      if (div_id == PSOC4_PCLK_DIV_NONE)
        return -ENOENT;

      if (ratio & ~div_mask[PSOC4_PCLK_DIV_TYPE(div_id)])
        return -ENOTSUP;
...
static DEV_USE(psoc4_pclk_use)
{
  ...
  case DEV_USE_CLOCK_SINK_GATE_DONE: {
    ...
    for (uint_fast8_t i = 0; i < PSOC4_PCLK_SRC_COUNT; ++i)
...
static DEV_INIT(psoc4_pclk_init)
{
  struct psoc4_pclk_private_s *pv = dev->drv_pv;
  ...
  for (uint_fast8_t i = 0; i < PSOC4_PCLK_SRC_COUNT; ++i) {
    dev_clock_source_init(dev, &pv->src[i], &psoc4_pclk_ep_setup);
    cpu_mem_write_32(PCLK + PCLK_PCLK_CTL_ADDR(i), pv->pclk_src[i]);
  }
  
  return 0;
}

static DEV_CLEANUP(psoc4_pclk_cleanup)
{
  struct psoc4_pclk_private_s *pv = dev->drv_pv;
...
#define psoc4_pclk_app_configid_set (dev_cmu_app_configid_set_t*)dev_driver_notsup_fcn

DRIVER_DECLARE(psoc4_pclk_drv, 0, "PSoC4 Pclk", psoc4_pclk,
               DRIVER_CMU_METHODS(psoc4_pclk));

DRIVER_REGISTER(psoc4_pclk_drv);
