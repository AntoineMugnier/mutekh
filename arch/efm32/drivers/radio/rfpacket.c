    ...
    if (irq & EFR32_PROTIMER_IF_LBTFAILURE)
    {
      ...
      if (!dev_rfpacket_is_packet_incoming(&ctx->gctx))
      {
        ...
        if (efr32_rfp_disable(ctx))
          efr32_rfp_req_timeout(ctx); // Disable already done, call req timeout
        else
          ctx->rac_irq_type = EFR32_RAC_IRQ_TIMEOUT; // Disable is pending irq
      }
      else
        efr32_rfp_req_timeout(ctx); // Call req timeout
    }
...
static void efr32_rfp_schedule_next_wup(struct radio_efr32_rfp_ctx_s *ctx,
                                        uint32_t rx_start, uint32_t rx_end)
{
  // Get current time
...
static void efr32_rfp_start_rx_ldc(struct radio_efr32_rfp_ctx_s *ctx)
{
  ...
  // Set rtcc config
  cpu_mem_write_32(EFM32_RTCC_ADDR + EFM32_RTCC_CC_CTRL_ADDR(1),
                   EFM32_RTCC_CC_CTRL_MODE_SHIFT_VAL(OUTPUTCOMPARE));
  cpu_mem_write_32(EFM32_RTCC_ADDR + EFM32_RTCC_CC_CTRL_ADDR(2),
                   EFM32_RTCC_CC_CTRL_MODE_SHIFT_VAL(OUTPUTCOMPARE));
  cpu_mem_write_32(EFM32_RTCC_ADDR + EFM32_RTCC_IFC_ADDR, endian_le32(EFM32_RTCC_IFC_MASK));
  cpu_mem_write_32(EFM32_RTCC_ADDR + EFM32_RTCC_IEN_ADDR, EFM32_RTCC_IEN_CC(2)
                   | EFM32_RTCC_IEN_CC(1));
...
static void efr32_rfp_start_tx_lbt(struct radio_efr32_rfp_ctx_s *ctx, struct dev_rfpacket_rq_s *rq)
{
  ...
  cpu_mem_write_32(EFR32_PROTIMER_ADDR + EFR32_PROTIMER_TOUTCNTTOP_ADDR(0),
                   EFR32_PROTIMER_TOUTCNTTOP_PCNTTOP(4));
  cpu_mem_write_32(EFR32_PROTIMER_ADDR + EFR32_PROTIMER_TOUTCOMP_ADDR(0),
                   EFR32_PROTIMER_TOUTCOMP_PCNTCOMP(1));
  ...
  cpu_mem_write_32(EFR32_PROTIMER_ADDR + EFR32_PROTIMER_LBTSTATE1_ADDR,
                   EFR32_PROTIMER_LBTSTATE1_EXP(1));
#endif
...
static void efr32_rfp_tx_irq(struct radio_efr32_rfp_ctx_s *ctx, uint32_t irq)
{
  ...
  if (irq & EFR32_FRC_IF_TXDONE)
    efr32_rfp_set_status_done(ctx, DEV_RFPACKET_STATUS_TX_DONE); // Set status
  else
  {
    // Clear buffer
  ...
  if (efr32_rfp_disable(ctx))
    efr32_rfp_req_done(ctx); // Call rfpacket to process request completion
  else
    ctx->rac_irq_type = EFR32_RAC_IRQ_TXRX; // Wait for rac irq
}
...
static error_t efr32_radio_check_config(struct dev_rfpacket_ctx_s *gpv,
...
static void efr32_radio_rx(struct dev_rfpacket_ctx_s *gpv,
...
static void efr32_radio_tx(struct dev_rfpacket_ctx_s *gpv,
                           struct dev_rfpacket_rq_s *rq, bool_t isRetry)
{
  struct radio_efr32_rfp_ctx_s *ctx = gpv->pvdata;
...
static DEV_INIT(efr32_radio_init)
{
  ...
  // Clock init
  if (dev_drv_clock_init(dev, &pv->pti.clk_ep, 0, DEV_CLOCK_EP_POWER_CLOCK
      | DEV_CLOCK_EP_GATING_SYNC, NULL))
    goto err_clk;

  for (uint8_t i = 0; i < EFR32_RADIO_CLK_EP_COUNT; i++)
  {
    if (dev_drv_clock_init(dev, &pv->clk_ep[i], i, DEV_CLOCK_EP_POWER_CLOCK
        | DEV_CLOCK_EP_GATING_SYNC, NULL))
      goto err_clk;
