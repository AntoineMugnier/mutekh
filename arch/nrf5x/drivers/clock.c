...
struct nrf5x_clock_context_s
{
  ...
  bool_t force[NRF_CLOCK_EP_COUNT];
#endif
  bool_t notifying;
...
static DEV_CLOCK_CONFIG_NODE(nrf5x_clock_config_node)
{
  return -ENOTSUP;
}
...
#endif
  case NRF_CLOCK_HF:
    hf_clock_mode_update(pv, pv->force[id]);
    break;

  case NRF_CLOCK_LF:
  case NRF_CLOCK_LF_PRECISE:
    lf_clock_mode_update(pv, pv->force[id]);
    break;
  }
...
static DEV_INIT(nrf5x_clock_init)
{
  ...
  kroutine_init(&pv->state_checker, nrf5x_clock_state_check, KROUTINE_IDLE);
