                               ...
                               struct net_layer_s **layer)
{
  ...
  ble_timing_mapper_master_init(&master->timing,
                                &self_as_timer,
                                &params->conn_req,
                            ...
                            enum event_status_e status)
{
  ...
  if (master->stuck_events_left == 0)
    nrf5x_ble_master_error(master, BLE_RESPONSE_TIMEOUT);
                               ...
                               enum nrf5x_ble_transfer_e mode)
{
  ...
  data[0] = (data[0] & 0x3)
    ...
    | (master->empty_first ? 0x20 : 0)
    | (__MIN(master->tx_queue_count, 3) << 6);
