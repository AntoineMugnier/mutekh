...
void nrf5x_ble_rtc_start(struct nrf5x_ble_private_s *pv)
{
  ...
  dev_clock_sink_hold(&pv->clock_sink[NRF5X_BLE_CLK_SLEEP], 1);
#endif
...
{
#if defined(CONFIG_DEVICE_CLOCK)
  dev_clock_sink_release(&pv->clock_sink[NRF5X_BLE_CLK_SLEEP]);
#endif
...
{
  if (!value)
    value = nrf_reg_get(BLE_RTC_ADDR, NRF_RTC_COUNTER) + 32768 * 2;
...
void nrf5x_ble_timer_init(void)
{
  ...
  nrf_short_set(BLE_TIMER_ADDR, (1 << NRF_TIMER_COMPARE_CLEAR(TIMER_IFS_TIMEOUT)));
  nrf_reg_set(BLE_TIMER_ADDR, NRF_TIMER_CC(TIMER_IFS_TIMEOUT),
              BLE_T_IFS
              + 8 + 32
              + RADIO_IRQ_LATENCY_US
              + RADIO_RX_CHAIN_DELAY_US
              + 20);
  nrf_reg_set(BLE_TIMER_ADDR, NRF_TIMER_CC(TIMER_ENABLE),
              1);
}
...
void nrf5x_ble_ppi_init(void)
{
  ...
  nrf_ppi_setup(PPI_ADDRESS_PIPELINE_RESET,
                BLE_RADIO_ADDR, NRF_RADIO_ADDRESS,
  ...
  nrf_reg_set(NRF_PPI_ADDR, NRF_PPI_CHG(PPI_GROUP_PIPELINE_RESET), 0
              ...
              | (1 << PPI_TIMER_TIMEOUT_DISABLE)
              | (1 << PPI_TIMER_ENABLE_TXEN)
              | (1 << PPI_TIMER_ENABLE_RXEN));

  nrf_ppi_enable(PPI_ADDRESS_PIPELINE_RESET);
}
...
error_t 
nrf5x_ble_data_setup(struct nrf5x_ble_private_s *pv)
{
  assert(pv->current);
...
bool_t nrf5x_ble_clock_request(struct nrf5x_ble_private_s *pv)
{
  if (!pv->accurate_clock_requested) {
    ...
    dev_clock_sink_hold(&pv->clock_sink[NRF5X_BLE_CLK_RADIO], 0);
  }
...
void nrf5x_ble_clock_release(struct nrf5x_ble_private_s *pv)
{
  if (pv->accurate_clock_requested) {
    ...
    dev_clock_sink_release(&pv->clock_sink[NRF5X_BLE_CLK_RADIO]);
  }
}

DEV_CLOCK_SINK_CHANGED(nrf5x_ble_clock_changed)
{
  struct nrf5x_ble_private_s *pv = ep->dev->drv_pv;
  uint8_t clk = ep - pv->clock_sink;

  if (clk == NRF5X_BLE_CLK_SLEEP
      && ep->src->flags & DEV_CLOCK_SRC_EP_RUNNING)
    pv->sleep_acc = *acc;

  if (clk == NRF5X_BLE_CLK_RADIO) {
    pv->accurate_clock_running = !!(ep->src->flags & DEV_CLOCK_SRC_EP_RUNNING);

    gpio(I_CLOCK_RUN, pv->accurate_clock_running ? I_CLOCK_RUN : 0);
  }
}
...
#if defined(CONFIG_ARCH_NRF52) && 0
